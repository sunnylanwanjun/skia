...
protected:
    ...
    void onOnceBeforeDraw() override {
        ...
        }
        {
            ...
            auto [spec, error] = SkMeshSpecification::Make(
                    kAttributes, sizeof(NoColorVertex), kVaryings, SkString(kVS), SkString(kFS));
            if (!spec) {
                SkDebugf("%s\n", error.c_str());
            }
    ...
    void ensureBuffers() {
        if (!fColorVB) {
            fColorVB = SkMesh::MakeVertexBuffer(
                    /*GrDirectContext*=*/nullptr, kColorQuad, sizeof(kColorQuad));
        }
        ...
        if (!fNoColorVB) {
            ...
            std::memcpy(SkTAddOffset<void>(data->writable_data(), kNoColorOffset),
                        kNoColorQuad,
                        sizeof(kNoColorQuad));
            fNoColorVB = SkMesh::MakeVertexBuffer(
                    /*GrDirectContext*=*/nullptr, data->data(), data->size());
        }
        ...
        if (!fColorIndexedVB) {
            ...
            fColorIndexedVB = SkMesh::MakeVertexBuffer(
                    /*GrDirectContext*=*/nullptr, data->data(), data->size());
        }

        if (!fNoColorIndexedVB) {
            fNoColorIndexedVB = SkMesh::MakeVertexBuffer(
                    /*GrDirectContext*=*/nullptr, kNoColorIndexedQuad, sizeof(kNoColorIndexedQuad));
        }
...
protected:
    ...
    void onOnceBeforeDraw() override {
        ...
        fShader = SkGradientShader::MakeRadial(
                kGradCenter, .4f, colors, nullptr, 2, SkTileMode::kMirror);
        ...
        for (bool gpuBuffer : {false, true}) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                for (size_t j = 0; j < std::size(kTri); ++j) {
                    SkAssertResult(vb->update(
                            ctx, &kTri[j], spec->stride() * j + posOffset, sizeof(kTri[j])));
                }
...
protected:
    ...
    void onOnceBeforeDraw() override {
        ...
        fShader = SkGradientShader::MakeSweep(kRect.centerX(),
                                              kRect.centerY(),
                                              colors,
                                              SkColorSpace::MakeSRGB(),
                                              nullptr,
