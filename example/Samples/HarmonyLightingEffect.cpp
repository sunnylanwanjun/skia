...
private:
    std::shared_ptr<SkRuntimeShaderBuilder> getBtnShaderBuilder() {
        ...
        if (!lightEffect) {
            SkDebugf("HarmonyLightingEffect::getBtnShaderBuilder MakeShader Failed\n");
            return nullptr;
        }
    ...
    std::shared_ptr<SkRuntimeShaderBuilder> getBorderShaderBuilder() {
        ...
        auto [lightEffect, error] = SkRuntimeEffect::MakeForShader(ligthString);
        if (!lightEffect) {
            SkDebugf("HarmonyLightingEffect::getBorderShaderBuilder MakeShader Failed\n");
            return nullptr;
        }
        lightEffect_ = std::move(lightEffect);
...
HarmonyLightingEffect::HarmonyLightingEffect(int argc, char** argv, void* platformData)
#if defined(SK_GL)
...
HarmonyLightingEffect::~HarmonyLightingEffect() {
    fWindow->detach();
    delete fWindow;
}

void HarmonyLightingEffect::updateTitle() {
    if (!fWindow) {
        return;
    }
...
void HarmonyLightingEffect::onBackendCreated() {
    this->updateTitle();
    fWindow->show();
    fWindow->inval();
}

void HarmonyLightingEffect::onPaint(SkSurface* surface) {
    ...
    // SkDebugf("HarmonyLightingEffect::onPaint layer size x=%d, y=%d\n", size.fWidth, size.fHeight);
...
void HarmonyLightingEffect::onIdle() {
    // Just re-paint continuously
    fWindow->inval();
}

bool HarmonyLightingEffect::onChar(SkUnichar c, skui::ModifierKey modifiers) {
    if (' ' == c) {
        if (Window::kRaster_BackendType == fBackendType) {
#if defined(SK_GL)
            fBackendType = Window::kNativeGL_BackendType;
...
bool HarmonyLightingEffect::onMouse(int x, int y, skui::InputState state, skui::ModifierKey modifiers) {
    mousePos = {float(x), float(y)};

    switch (state) {
        case skui::InputState::kUp: {
            // SkDebugf("HarmonyLightingEffect::onMouse Up x=%d, y=%d\n", x, y);
            break;
        }
        case skui::InputState::kDown: {
            // SkDebugf("HarmonyLightingEffect::onMouse Down x=%d, y=%d\n", x, y);
            break;
        }
        case skui::InputState::kMove: {
            // SkDebugf("HarmonyLightingEffect::onMouse Move x=%d, y=%d\n", x, y);
            break;
        }
        default: {
